...
So, it contains the following parts:
1) it should take parameters for the connection to the remote host: login, ip (use keys to connect)
...
# also I can check the differencies of implementations for macos and linux on the python described in the py documentation

3) it connects via ssh to the remote host
# it may need password, but I think it is not secure, so I omit this option
# this programm works only with public key on the remote host
...
# * and maybe it's downloaded local, and then it is copied to the remote host
# TODO: look for installation instruction
# so here: download - install or download - copy - install
...
if it is frresh install, test installation: 
